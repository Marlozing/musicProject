...
from musicProject.tests.SelfFindStartTime import SelfAdjuster
from musicProject.src.services.crawlService import CrawlService
...
class OptionWindow:
    ...
    def read_audio(self):
        ...
        tk.Label(root, text="Read Audio").grid(
...
class Main:
    def __init__(self):
        ...
        # 숫자 길이 입력
        ttk.Label(self.root, text="Read Page Num:").grid(
            row=2, column=0, padx=5, pady=5
        )
        self.length_var = tk.IntVar(value=1)
        self.length_spinbox = ttk.Spinbox(
            self.root, values=list(range(1, 11)), width=5, textvariable=self.length_var
        )
        self.length_spinbox.grid(row=2, column=1, padx=5, pady=5)

        try:
            with open("data/previous_setting.txt", "r") as f:
        ...
        self.remember_checkbox = ttk.Checkbutton(
            self.root, text="Remember", variable=self.remember_var
        )
        self.remember_checkbox.grid(row=3, column=0, columnspan=2, pady=5)

        # 제출 버튼
        self.submit_button = ttk.Button(
            self.root, text="Submit", command=self.submit_data
        )
        self.submit_button.grid(row=4, column=0, columnspan=2, pady=10)

        # Previous Setting 버튼
        self.previous_button = ttk.Button(
            self.root, text="Use Previous Setting", command=self.download_audio
        )
        self.previous_button.grid(row=5, column=0, columnspan=2, pady=10)

        # Audio 파일 보기
        self.audio_button = ttk.Button(
            self.root, text="Audio Files", command=self.select_audio
        )
        self.audio_button.grid(row=6, column=0, columnspan=2, pady=10)

        # 수동 맞춤
        self.self_adjust_button = ttk.Button(
            self.root, text="Self Adjust", command=self.self_adjust
        )
        self.self_adjust_button.grid(row=7, column=0, columnspan=2, pady=10)
    ...
    def submit_data(self):
        ...
        if remember:
            with open("data/previous_setting.txt", "w") as f:
                f.write(f"{user_id}\n{password}\n{length}")
...
if __name__ == "__main__":
    ...
    async def main():
        ...
        a = await crawlService.downVideo("https://cafe.naver.com/steamindiegame/18180324")
        print(a)
