...
class SelfAdjuster:
  def __init__(self):
    ...
    self.origin_path = "audio/원본.wav" if os.path.exists("../datas/audio/원본.wav") else None
    self.compare_path = None

    ttk.Label(self.root, text="Read Audio").grid(row=0, column=0, columnspan=2, padx=5, pady=5)

    self.get_origin_button = tk.Button(self.root, text="원본 파일 열기", command=self.get_origin)
    self.get_origin_button.grid(row=1, column=0, padx=5, pady=5)

    self.get_compare_button = tk.Button(self.root, text="반응 파일 열기", command=self.get_compare)
